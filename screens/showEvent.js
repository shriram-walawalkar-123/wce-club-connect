    ...
    <ScrollView className="bg-gray-100 p-4">
      ...
      {event.subEvents.map((subEvent, index) => (
        <View key={index} className="bg-white p-6 rounded-lg mb-6 shadow-lg border border-gray-200">
          ...
          {/* Venue */}
          <Text className="text-base text-gray-700 mb-2">{`Venue: ${subEvent.venue}`}</Text>
          
          {/* Entry Fee */}
          <Text className="text-base text-gray-700 mb-2">{`Entry Fee: ₹${subEvent.entryFee}`}</Text>

          {/* Rulebook Link */}
          <Text
            className="text-base text-blue-600 underline mb-4"
            onPress={() => Linking.openURL(subEvent.rulebookPDF)}
          >
